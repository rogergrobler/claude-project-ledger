...
- **Python-then-Edit pattern**: writing to `current.html` via a Python subprocess (e.g. for bulk regex-replacement of the priority-grid block) invalidates the Edit tool's file-state cache for that file. Any Edit calls issued after a Python write to the same file will fail with "File has not been read yet" until a fresh Read is issued. This has bitten the v1.24 FP grid replace, the v1.25 ship, and the v1.26 Pixel-payload application. The fix: after any Python write to current.html, immediately Read at least one line of the file before issuing further Edits, OR batch all Python-style bulk rewrites BEFORE any tool-level Edit. Don't interleave.
